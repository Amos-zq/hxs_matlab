...
function EEG = pas_sim(EEG,QRSevents,method,npc)
...
case 'sim'
    ...
    % Calculate Component Using SIM
    epochrange=round(1.5*median(diff(peakI))/2); % half RR
    epocheddata = epoch( EEG.data, peakI, [-epochrange epochrange+1]);
    [simA,simS,simz] = SIM(double(epocheddata));
    simP = zeros(size(epocheddata,1), size(epocheddata,2),npc);
    for i = 1:npc
        simP(:,:,i) = simA(:,i)*simz(i,:);
    end
